...
class ShuffleNetV2(nn.Module):
    def __init__(self, stage_out_channels, load_param, all_output=False, data_channel=3):
        ...
        self.first_conv = nn.Sequential(
            nn.Conv2d(data_channel, input_channel, 3, 2, 1, bias=False),
    ...
    def set_output(self, all_output=False):
        self.all_output = all_output

    def _initialize_weights(self):
        print("initialize_weights...")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # self.load_state_dict(torch.load("./model/backbone/backbone.pth", map_location=device), strict = False)
